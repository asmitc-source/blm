...
describe("redirectToLoginIfRequired", () => {
    ...
    const did = withWindow(
      ...
      () =>
        redirectToLoginIfRequired({
          ...
          data: null,
          loginRequired: true,
          loginUrl: "https://auth.example.com/signin?return_to=x",
        }),
    );
    assert.equal(did, true);
    assert.equal(target, "https://auth.example.com/signin?return_to=x");
  });
  ...
  it("opens a new tab instead of navigating when framed", () => {
    ...
    assert.equal(did, true);
    assert.equal(opened, "https://auth.example.com/signin?return_to=x");
  ...
  it("falls back to navigation when framed and the popup is blocked", () => {
    ...
    assert.equal(did, true);
    assert.equal(assigned, "https://auth.example.com/signin?return_to=x");
  });
