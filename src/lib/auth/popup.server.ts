...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // redirect_uri for the dynamic `the preview host` origin.
...
<script type="application/json" id="blm-auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("blm-auth-popup-msg");
  var msg = { source: "blm-auth-popup", token: null };
