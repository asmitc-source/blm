...
export const cmsLibraryStatus = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    requireAdmin(context.admin);
    const { BLOG_POSTS } = await import("@/lib/content/blog");
    const { listArticles, expectedLibrarySlugs } = await import("./store");
...
/** One round-trip for /admin/articles: auth + list + missing library slugs. */
export const cmsArticlesPage = createServerFn({ method: "GET" })
  .middleware([deskMiddleware])
  .handler(async ({ context }) => {
    if (!context.admin) {
...
export const cmsSaveArticle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    // Previous status is best-effort — never block publish on a slow read.
    let wasPublished = false;
    if (data.id) {
      try {
        const previous = await Promise.race([
          getArticle(data.id),
